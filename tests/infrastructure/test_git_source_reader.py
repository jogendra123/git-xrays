...
class TestGitSourceReader:
    def test_list_python_files_finds_py_files(self, anemia_repo: Path):
        reader = GitSourceReader(str(anemia_repo))
        files = reader.list_python_files()
        assert any(f.endswith(".py") for f in files)

    def test_list_python_files_excludes_non_py(self, anemia_repo: Path):
        reader = GitSourceReader(str(anemia_repo))
        files = reader.list_python_files()
        assert all(f.endswith(".py") for f in files)

    def test_read_file_returns_content(self, anemia_repo: Path):
        reader = GitSourceReader(str(anemia_repo))
        files = reader.list_python_files()
        content = reader.read_file(files[0])
        assert len(content) > 0

    def test_read_file_at_ref(self, anemia_repo: Path):
        reader = GitSourceReader(str(anemia_repo))
